...
hulf_stuff_file = "#{node['jenkins']['hulf_stuff_atr']}_for_glob"
atr_in_encode_file = "#{node['jenkins']['encode_atr']}_for_glob"
my_string = ''
uri = node['jenkins']['url']

ruby_block "timer4" do
  block do
    sleep 10.0
  end
end  
...
execute "update_glob_script" do
  command "grep -v '^$' #{hulf_stuff_file} > #{atr_in_encode_file}"
  action :run
  notifies :run, 'ruby_block[make_string_for_glob]', :immediately
  notifies :run, 'execute[post_request_for_glob]', :immediately
end 

ruby_block "make_string_for_glob" do
  block do
...
execute "post_request_for_glob" do
  command "curl -d @#{atr_in_encode_file} #{uri}/configSubmit"
  action :nothing
end
